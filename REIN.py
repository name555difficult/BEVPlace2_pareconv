...
class REM(nn.Module):
    def __init__(self, from_scratch=False, rotations=8):
        ...
        # rotations
        self.angles = -torch.arange(0,359.00001,360.0/rotations)/180*math.pi
    ...
    def forward(self, x):
        ...
        for i in range(len(self.angles)):
            ...
            # input warp
            warped_im = F.grid_sample(x, grid,align_corners=True,mode='bilinear')
            ...
            # output feature warp    
            out = F.grid_sample(out, grid ,align_corners=True,mode='bilinear')
        ...
        out1 = F.grid_sample(equ_features, grid,align_corners=True,mode='bilinear')
        out1 = F.normalize(out1, dim=1)
        
        # upsample for keypoints
        grid = F.affine_grid(aff, torch.Size((B, C, H, W)),align_corners=True).type(x.type())#,align_corners=True)
        out2 = F.grid_sample(equ_features, grid,align_corners=True,mode='bilinear')
