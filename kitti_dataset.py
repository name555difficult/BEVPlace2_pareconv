...
from collate import multi_single_pc_collate_fn_stack_mode
...
def pc_collate_fn(batch):
    ...
    point_cloud_list = []
    point_cloud_list.append(torch.from_numpy(query[0]).float())
    point_cloud_list.append(torch.from_numpy(positive[0]).float())
    point_cloud_list += [
        torch.from_numpy(negatives[0][i]).float() for i in range(len(negatives[0]))
    ]
...
class TrainingDataset(data.Dataset):
    def __init__(self, dataset_path = './datasets/KITTI/',seq='00', augmentation_rotation=1.0, num_neg=10):
        super().__init__()
    ...
    def get_pointcloud(self, imgpath):
    ...
    def __getitem__(self, index):
        ...
        pc_query = np.matmul(pc_query, aug_rotation.T)
        # rot augmentation
        ...
        pc_positive = self.get_pointcloud(self.imgs_path[pos_idx])
        aug_rotation = random_sample_rotation(self.augmentation_rotation)
        pc_positive = np.matmul(pc_positive, aug_rotation.T)
        ...
        for neg_i in neg_idx:
            ...
            pc_negative = self.get_pointcloud(self.imgs_path[neg_i])
            aug_rotation = random_sample_rotation(self.augmentation_rotation)
            pc_negative = np.matmul(pc_negative, aug_rotation.T)
            ...
            pc_negatives.append(pc_negative)

        # negatives = torch.stack(negatives, 0)

        return pc_query, pc_positive, pc_negatives, index
